...
const mongoose = require('mongoose'),
      ...
      generateClassifierWithSurroundingWordsAndPOSTags = classifier.generateClassifierWithSurroundingWordsAndPOSTags,
...
.then(() => {
  ...
  generateClassifierWithoutPhrase(classifier => {
    Line.find({ dataType: 'dev-data'}, { _id: 0, lineId: 0, updatedAt: 0, createdAt: 0, __v: 0 }, (err, lines) => {
      ...
      console.log(`The all words (except phrase) classifier has ${(accuracy*100).toFixed(2)}% accuracy.`);
    });
  });
  ...
  generateClassifierWithSurroundingWords(classifier => {
    Line.find({ dataType: 'dev-data'}, { _id: 0, lineId: 0, updatedAt: 0, createdAt: 0, __v: 0 }, (err, lines) => {
      ...
      const accuracy = numHits / numLines;
      console.log(`The surrounding words classifier has ${(accuracy*100).toFixed(2)}% accuracy.`);
    });
  });
  ...
  generateClassifierWithSurroundingPOSTags(classifier => {
    const tagger = new Tagger(lexiconFile, rulesFiles, defaultCategory, error => {
      ...
      Line.find({ dataType: 'dev-data' }, { _id: 0, lineId: 0, updatedAt: 0, createdAt: 0, __v: 0 }, (err, lines) => {
        ...
        const accuracy = numHits / numLines;
        console.log(`The surrounding POS tags classifier has ${(accuracy*100).toFixed(2)}% accuracy.`);
      });
    });
  });

  generateClassifierWithSurroundingWordsAndPOSTags(classifier => {
    const tagger = new Tagger(lexiconFile, rulesFiles, defaultCategory, error => {
      if (error) console.error(error.message);

      Line.find({ dataType: 'dev-data' }, { __v: 0, id: 0, lineId: 0, updatedAt: 0, createdAt: 0 }, (err, lines) => {
        if (err) console.error(err.message);
        ...
        const accuracy = numHits / numLines;
        console.log(`The surrounding words and POS tags classifier has ${(accuracy*100).toFixed(2)}% accuracy.`);
      });
    });
  });
});
